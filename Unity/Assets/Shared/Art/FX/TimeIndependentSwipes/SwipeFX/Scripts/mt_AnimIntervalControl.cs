  ...
  void RunAnimations (){
    ...
    if (t > 0) {
      ...
      while (tempT < t) {
        ...
        m                 = tempT / t          ;
        xform.eulerAngles = new Vector3(Mathf.LerpAngle(lastEulerAngles.x, eulerAngles.x, m),Mathf.LerpAngle(lastEulerAngles.y, eulerAngles.y, m),Mathf.LerpAngle(lastEulerAngles.z, eulerAngles.z, m));
        xform.position    = Vector3.Lerp(lastPosition, position, m);
