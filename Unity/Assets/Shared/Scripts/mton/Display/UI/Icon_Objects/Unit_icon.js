...
function Start () { //can't be on script Awake(), Unit_icon_Spawner is generating the icon Nodes
	...
	/* ---DELETE ME-----
	//renderer.material.renderQueue = 30001; //makes sure icon node is rendered last 
	//3000 is default transparency layer in Unity
	//Using shader with ZBuffer turned off, this only affects render order
	*/
}
...
function iconToggle(bool:boolean){
	...
	/* ---DELETE ME----- More complicated than just turning off Collider component
	if(bool===false){
		transform.gameObject.layer = 2; // Put the game object in the ignore raycast layer
	}
	else{
		transform.gameObject.layer = 0; // Default to default layer
	}
	*/
}
