...
#pragma strict 
//downcast and implicit are for backwards compatibility in unity 3
#pragma implicit
#pragma downcast
...
class mt_Car extends mt_OnHitDeath{
  //maximal corner and braking acceleration capabilities
  var maxCornerAccel  = 10.0 ;
  var maxBrakeAccel   = 10.0 ;
  var idleRBodyDrag   = 0.25 ; //how much rigidbody drag to apply when not motoring - prevents slipping : 0.0 for ice
  var idleRBodyTorque = 75   ; //how much rigidbody drag to apply when not motoring - prevents slipping : 0.0 for ice
  ...
  var frictionWheelFwd = 0.25 ;
  var frictionWheelSde = 0.25 ;
  ...
  //private var queryUserInput = true   ;  //already accounted for in mt_GameObject class.
  ...
  function GetStatus(gui : GUIText) {
    gui.text="v="+(rbody.velocity.magnitude * 3.6).ToString("f1") + " km/h\ngear= "+gear+"\nrpm= "+engineRPM.ToString("f0");
  }
  ...
  function Start () {
    ...
    for(w in wheels){
      ...
      w.coll.forwardFriction.stiffness  = frictionWheelFwd           ;
      w.coll.sidewaysFriction.stiffness = frictionWheelSde           ;
  ...
  function UpdateWheels(){
    ...
    handbrakeSlip=handbrake*rbody.velocity.magnitude*0.1;
    if(handbrakeSlip>1)
      handbrakeSlip=1;
    ...
    for(w in wheels){		
      ...
      if(c.GetGroundHit(hit)){
        ...
        if(slip>0.5 && hit.collider.tag=="Dusty"){
          ...
          //groundDustEffect.particleEmitter.worldVelocity=rbody.velocity*0.5;
  ...
  function HandlePhysics () {
    ...
    rbody.angularVelocity = new Vector3(rbody.angularVelocity.x,0.0,rbody.angularVelocity.z) ;
    ...
    brakeForce                   = -flatVelo.normalized*totalbrake*rbody.mass*maxBrakeAccel ;
    ...
    cornerForce                     = -(flatVelo-flatDir).normalized*cornerAccel*rbody.mass ;
    cornerSlip                      = Mathf.Pow(cornerAccel/maxCornerAccel,3)               ;

    rbody.AddForceAtPosition(brakeForce+engineForce+cornerForce,transform.position+transform.up*wheelY);
  ...
  function FixedUpdate () {
    ...
    if(motor<0.001){ 
    	if(onGround){  //allow turning while sitting still
    		...
    		rbody.AddTorque(rbody.transform.up * steer * idleRBodyTorque, ForceMode.Acceleration) ;
    	}
    }
    else{
        rbody.drag = 0.00;
