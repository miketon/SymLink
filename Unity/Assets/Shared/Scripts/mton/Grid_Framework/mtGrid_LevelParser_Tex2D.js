...
class mtGrid_LevelParser_Tex2D extends mtGrid_LevelParser{
  ...
  function ReadLevelData(LevelData:Texture2D){
    ...
    SetGridRangeLevel(Vector3(0,0,0), Vector3(imgWdth, imgHght, 0)); //Bottom Right corner
    ...
    for(var h:int=0; h<imgHght; h++){ //starts at bottom left
      for(var w:int=0; w<imgWdth; w++){
        ...
        if(gPlane == GFGrid.GridPlane.XZ){
          targetPosition = GRID_mt.GridToWorld(Vector3(w+gOffSet, 0.0, (-(imgHght-1)+h-gOffSet)+imgHght)) ;
        }
        else if(gPlane == GFGrid.GridPlane.XY){
          targetPosition = GRID_mt.GridToWorld(Vector3(w+gOffSet, (-(imgHght-1)+h-gOffSet)+imgHght, 0.0)) ;
        }
        else if(gPlane == GFGrid.GridPlane.YZ){
          targetPosition = GRID_mt.GridToWorld(Vector3(0.0, w+gOffSet, (-(imgHght-1)+h-gOffSet)+imgHght)) ;
        }
