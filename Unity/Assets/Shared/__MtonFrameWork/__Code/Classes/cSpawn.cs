...
namespace MTON.Class{
  public class cSpawn : MonoBehaviour {
    ...
    public delegate void OnEMIT(Transform IN_XFORM, int IN_INDEX) ; //set up delegate
	...
	public virtual void doEmit(Transform IN_XFORM, int IN_INDEX){ //on Bullet Emission (projectile)
	  if(OnEmitDelegate != null){ // NOTE: Just in case class exist, but no delegate is assigned
	    OnEmitDelegate(IN_XFORM, IN_INDEX);
      }
	}

	public virtual void doFlar(Transform IN_XFORM, int IN_INDEX){ //on Firing Point Emission (flare)
	  if(OnFlarDelegate != null){ // NOTE: Just in case class exist, but no delegate is assigned
	    OnFlarDelegate(IN_XFORM, IN_INDEX);
      }
	}
    ...
    public virtual void doAttk(bool bAttk, bool IN_FACEFORWARD=true){
      if(bAttk){
        if(this.sEM.firePnts.Length > 0){
		  ...
		  int indexFP = this.sFP_mod.iIndex;
