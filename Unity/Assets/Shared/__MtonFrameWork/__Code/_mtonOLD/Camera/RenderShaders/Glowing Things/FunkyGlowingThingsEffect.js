...
private static var blurMatString =
	"Shader \"BlurConeTap\" { SubShader { Pass { " +
		"ZTest Always Cull Off ZWrite Off Fog { Mode Off } " +
		//"Color (1,0,0,0)"+
		"	SetTexture [__RenderTex] {constantColor (0,0,0,0.5) combine texture * constant alpha} " +
		"	SetTexture [__RenderTex] {constantColor (0,0,0,0.5) combine texture * constant + previous} " +
		//"	SetTexture [__RenderTex] {constantColor (0,0,0,0.5) combine texture * constant + previous} " +
	"} } Fallback off }";
...
private function GetMaterial() : Material {
	if (m_Material == null) {
		m_Material = new Material( blurMatString ); //Create a temporary material from a shader source string.
...
function OnPreRender()
{
	...
	renderTexture = RenderTexture.GetTemporary (GetComponent.<Camera>().pixelWidth * 0.25, GetComponent.<Camera>().pixelHeight * 0.25, 16); //Allocate a temporary render texture.
...
function OnRenderImage (source : RenderTexture, destination : RenderTexture) : void{
	...
	var mtonTime :float = (Mathf.Cos(Time.frameCount*0.02) * 0.5) - 1.0;
