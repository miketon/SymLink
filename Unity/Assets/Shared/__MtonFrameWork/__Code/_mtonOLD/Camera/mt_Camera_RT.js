...
function initRendTexture(){
  renderTexture            = RenderTexture.GetTemporary (GetComponent.<Camera>().pixelWidth, GetComponent.<Camera>().pixelHeight, 8) ; //Allocate a temporary render texture.
...
function initRendTexture_m(){
  renderTexture_m            = RenderTexture.GetTemporary (GetComponent.<Camera>().pixelWidth * scaleResolution, GetComponent.<Camera>().pixelHeight * scaleResolution, 8) ; //Allocate a temporary render texture.
...
function initShaderCam(shaderCamera : Camera, renderTexture : RenderTexture):Camera{
  ...
  if (!shaderCamera) {
    ...
    shaderCamera           = tempObj.GetComponent.<Camera>()                         ;
